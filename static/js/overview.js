(function(){
  var _b=null;
  function _a(b){['services','missing','risks','roadmap'].forEach(function(t){
    var e=document.querySelector('.exec-tab[data-tab="'+t+'"]');if(e)e.style.display=b?'':'none';
  });}
  async function _f(){try{
    var d=await fetch('/api/project-info').then(function(r){return r.json();});
    _b=d.is_bog!==false;
    if(window.AppState){if(!AppState._overviewData)AppState._overviewData={};AppState._overviewData.is_bog=_b;}
    _a(_b);
  }catch(e){}}
  ...
  function s(){obs.observe(document.body||document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['style']});}
...
async function _fetchEffortCached(phase) {
  ...
  try {
    ...
    if (hit) { const d = JSON.parse(hit); if (d?.months?.length) return d; }
  } catch(e) {}
...
// ─── Save KPI snapshot to sessionStorage ─────────────────────────────
function _saveKPISnapshot(phase, data) {
  try { sessionStorage.setItem('_kpi_' + phase, JSON.stringify(data)); } catch(e) {}
}
function _loadKPISnapshot(phase) {
  try { const h = sessionStorage.getItem('_kpi_' + phase); return h ? JSON.parse(h) : null; } catch(e) { return null; }
}
...
async function _loadPhaseCostKPIs() {
  // Pre-load snapshots if variance not loaded yet
  if (!AppState._latestProfData) {
    ...
    ['services','support','development','consultation'].forEach(ph => {
      ...
      if (snap) AppState._latestProfData[ph] = snap;
    });
  }
  ...
  for (const phase of phases) {
    try {
      ...
      // EAC: use value saved by variance tab, fallback to cumCostSAR
  ...
  // as-of + % complete from variance month
  try {
    ...
    const asOf = lmk ? 'as of '+lmk : '';
    const pctTxt = avg>0 ? avg+'% complete' : '';
    ['kpiCostAsOf','kpiEACAsOf'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=asOf;});
    ['kpiCostPct','kpiEACPct'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=pctTxt;});

    // Save snapshot so it persists across tab switches
    const isBogS = AppState._overviewData?.is_bog !== false;
    const phsS   = isBogS ? ['consultation','development'] : ['services','support'];
    phsS.forEach(ph => {
      const snap = AppState._latestProfData?.[ph];
      if (snap) _saveKPISnapshot(ph, snap);
    });
  } catch(e) {}
}
...
function renderPhaseFilters(phaseGroup, available) {
  ...
  const lbl = active.length===0||active.length===total ? `All phases (${total})`
              : active.length===1 ? active[0] : `${active.length} of ${total} phases`;
  ...
  html += '<span style="margin-left:8px;color:var(--text-muted);">▼</span></button>';
...
function renderEmployeeFilter(employees) {
  ...
  html += '</div>';
  const _seen=new Set(), _dedup=employees.filter(e=>{const k=e.replace(/^\[E\d+\]\s*/,'').toLowerCase().trim();if(_seen.has(k))return false;_seen.add(k);return true;});
  _dedup.forEach(e => {
