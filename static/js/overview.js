(function(){var _b=null;function _a(b){['services','missing','risks','roadmap'].forEach(function(t){var e=document.querySelector('.exec-tab[data-tab="'+t+'"]');if(e)e.style.display=b?'':' none';});}async function _f(){try{var d=await fetch('/api/project-info').then(function(r){return r.json();});_b=d.is_bog!==false;if(window.AppState){if(!AppState._overviewData)AppState._overviewData={};AppState._overviewData.is_bog=_b;}_a(_b);}catch(e){}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',_f);else _f();var obs=new MutationObserver(function(){if(_b===false)_a(false);});function s(){obs.observe(document.body||document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['style']});}if(document.body)s();else document.addEventListener('DOMContentLoaded',s);})();
/* Overview tab — Roadmap KPIs + Tasks Analysis with multi-phase + employee filter */

async function _fetchEffortCached(phase){var key='_eff_'+phase+'_'+(window._activeProjectId||AppState._overviewData?.project_id||'');try{var h=sessionStorage.getItem(key);if(h){var d=JSON.parse(h);if(d&&d.months&&d.months.length)return d;}}catch(e){}var res=await fetch('/api/effort/'+phase+'/all-months');if(!res.ok)throw new Error('Effort API '+res.status);var d=await res.json();try{sessionStorage.setItem(key,JSON.stringify(d));}catch(e){}return d;}
...
async function _loadPhaseCostKPIs() {
  ...
  try{const pr=await fetch('/api/overview/phase-progress');const pd=await pr.json();const isBogK=AppState._overviewData?.is_bog!==false;const phsK=isBogK?['consultation','development']:['services','support'];let lmk='',tc=0,cc=0;for(const ph of phsK){const mk=pd.phases?.[ph]?.month_key||'';if(mk&&mk>lmk)lmk=mk;const pct=pd.phases?.[ph]?.completion||0;if(pct>0){tc+=pct;cc++;}}const avg=cc>0?Math.round(tc/cc):0;['kpiCostAsOf','kpiEACAsOf'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=lmk?'as of '+lmk:'';});['kpiCostPct','kpiEACPct'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=avg>0?avg+'% complete':'';});}catch(e){}
}
...
function renderPhaseFilters(phaseGroup,available){const cont=document.getElementById('ovPhaseFilters');if(!cont)return;if(!available.length){cont.innerHTML='';return;}const total=available.length,active=AppState.activePhases||[];const lbl=active.length===0||active.length===total?`All phases (${total})`:active.length===1?active[0]:`${active.length} of ${total} phases`;cont.style.cssText='position:relative;display:inline-flex;flex-direction:column;';let html='<label class="filter-label">PHASES (multi-select)</label>';html+='<div style="position:relative;display:inline-block;" id="ovPhaseDropdown">';html+=`<button type="button" class="phase-toggle" id="ovPhaseToggle"><span id="ovPhaseLabel">${lbl}</span><span style="margin-left:8px;color:var(--text-muted);">▼</span></button>`;html+='<div id="ovPhaseMenu" style="display:none;position:absolute;top:100%;left:0;z-index:9999;min-width:280px;max-width:420px;max-height:360px;overflow-y:auto;background:white;border:1px solid var(--border);border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,.12);margin-top:2px;">';html+='<div class="phase-menu-actions"><a id="ovPhaseAll">Select all</a><a id="ovPhaseNone">Clear</a></div>';available.filter(p=>p&&p!=='undefined'&&p!=='null').forEach(p=>{const checked=active.includes(p),label=p==='No Phase'?'📭 No Phase':p;html+=`<label class="phase-option ${checked?'selected':''}" data-phase="${encodeURIComponent(p)}" style="display:flex;align-items:center;gap:8px;padding:6px 12px;cursor:pointer;"><input type="checkbox" ${checked?'checked':''}><span dir="auto">${label}</span></label>`;});html+='</div></div>';cont.innerHTML=html;const toggle=document.getElementById('ovPhaseToggle'),menu=document.getElementById('ovPhaseMenu');toggle.addEventListener('click',e=>{e.stopPropagation();menu.style.display=menu.style.display==='none'?'block':'none';toggle.classList.toggle('open',menu.style.display==='block');});document.addEventListener('click',e=>{if(!document.getElementById('ovPhaseDropdown')?.contains(e.target)){menu.style.display='none';toggle.classList.remove('open');}},{capture:false});menu.querySelectorAll('.phase-option').forEach(opt=>{opt.addEventListener('click',e=>{e.stopPropagation();const phase=decodeURIComponent(opt.dataset.phase);const cb=opt.querySelector('input');if(e.target!==cb)cb.checked=!cb.checked;AppState.activePhases=AppState.activePhases||[];if(cb.checked){if(!AppState.activePhases.includes(phase))AppState.activePhases.push(phase);opt.classList.add('selected');}else{AppState.activePhases=AppState.activePhases.filter(p=>p!==phase);opt.classList.remove('selected');}const a=AppState.activePhases;document.getElementById('ovPhaseLabel').textContent=a.length===0||a.length===total?`All phases (${total})`:a.length===1?a[0]:`${a.length} of ${total} phases`;loadTaskAnalysis(phaseGroup);});});document.getElementById('ovPhaseAll').addEventListener('click',e=>{e.stopPropagation();AppState.activePhases=[...available];renderPhaseFilters(phaseGroup,available);loadTaskAnalysis(phaseGroup);});document.getElementById('ovPhaseNone').addEventListener('click',e=>{e.stopPropagation();AppState.activePhases=[];renderPhaseFilters(phaseGroup,available);loadTaskAnalysis(phaseGroup);});}
...
function renderEmployeeFilter(employees) {
  ...
  html += '</div>';
  const empSeen=new Set(),ded=employees.filter(e=>{const k=e.replace(/^\[E\d+\]\s*/,'').toLowerCase().trim();if(empSeen.has(k))return false;empSeen.add(k);return true;});ded.forEach(e=>{const checked=(AppState.activeEmployees||[]).includes(e);html+=`<label class="phase-option ${checked?'selected':''}" data-emp="${encodeURIComponent(e)}"><input type="checkbox" ${checked?'checked':''}><span dir="auto">${e}</span></label>`;});
  html += '</div></div>';
  cont.innerHTML = html;
...
function renderTaskList(phaseGroup) {
  ...
  if(rem>0||AppState._taskPage>0){html+=`<div style="display:flex;gap:8px;justify-content:center;padding:14px 0;">`;if(rem>0)html+=`<button onclick="_ovLoadMore()" style="padding:6px 20px;border:1px solid var(--border);border-radius:6px;background:var(--bg-subtle);cursor:pointer;font-size:13px;font-weight:600;color:var(--navy);">Show 10 more <span style="color:var(--muted);font-size:12px;">(${rem} remaining)</span></button>`;if(AppState._taskPage>0)html+=`<button onclick="_ovCollapse()" style="padding:6px 16px;border:1px solid var(--border);border-radius:6px;background:transparent;cursor:pointer;font-size:12px;color:var(--muted);">↑ Collapse to 10</button>`;html+=`</div>`;}
  html+=`</div></div>`;
  cont.innerHTML = html;
