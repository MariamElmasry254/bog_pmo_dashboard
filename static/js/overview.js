...
async function _loadPhaseCostKPIs() {
  ...
  try{const pr=await fetch('/api/overview/phase-progress');const pd=await pr.json();
    const isBogK=AppState._overviewData?.is_bog!==false;const phsK=isBogK?['consultation','development']:['services','support'];
    let lmk='',tc=0,cc=0;
...
function renderPhaseFilters(phaseGroup, available) {
  ...
  menu.querySelectorAll('.phase-option').forEach(opt=>{
    opt.addEventListener('click',e=>{
      e.stopPropagation();const phase=decodeURIComponent(opt.dataset.phase);const cb=opt.querySelector('input');
      if(e.target!==cb)cb.checked=!cb.checked;
...
function phaseLabel(phases,total){
  ...
  if(!phases||phases.length===0)return `All phases (${t})`;if(phases.length===t)return `All phases (${t})`;
  if(phases.length===1)return phases[0];return `${phases.length} of ${t} phases`;
}
...
function renderEmployeeFilter(employees) {
  ...
  html += '</div>';
  const empSeen=new Set(),dedupedEmps=employees.filter(e=>{const k=e.replace(/^\[E\d+\]\s*/,'').toLowerCase().trim();if(empSeen.has(k))return false;empSeen.add(k);return true;});
