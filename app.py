...
@app.route('/api/global/travel/parse-ticket', methods=['POST'])
def api_parse_ticket():
    """Parse a flight ticket PDF and extract passenger name + travel dates.
    Accepts multipart file upload (field: 'file') or base64 JSON body.
    Returns: {name, start_date, end_date, direction, airline, flight}
    """
    import re, io, base64
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return jsonify({'error': 'PDF parsing library not available'}), 500
    ...
    name = ''
    # Pattern: "Passenger Name : MR . Kareem Elsafy - ADT"
    m = re.search(r'Passenger Name\s*:\s*(?:MR\.?|MS\.?|MRS\.?|DR\.?)?\s*([A-Z][A-Za-z ]+?)\s*[-–]\s*(?:ADT|CHD|INF|\w{2,4})\b', text)
    if m:
        raw_name = m.group(1).strip()
        # Convert "KAREEM ELSAFY" → "Kareem Elsafy"
        name = ' '.join(w.capitalize() for w in raw_name.split())

    if not name:
        # Fallback: "Name : MR . Kareem Elsafy - ADT"
        m2 = re.search(r'Name\s*:\s*(?:MR\.?|MS\.?|MRS\.?)?\s*\.?\s*([A-Za-z][A-Za-z ]+?)\s*[-–]\s*ADT', text)
        if m2:
            name = ' '.join(w.capitalize() for w in m2.group(1).strip().split())

    # ── Parse dates ─────────────────────────────────────────────────
    # Pattern: "26Jan2026" or "26 Jan 2026"
    MONTHS = {
        'jan':1,'feb':2,'mar':3,'apr':4,'may':5,'jun':6,
        'jul':7,'aug':8,'sep':9,'oct':10,'nov':11,'dec':12
    }

    def parse_flight_date(s):
        s = s.strip()
        # "26Jan2026"
    ...
    all_dates = re.findall(r'\d{1,2}[A-Za-z]{3}\d{4}', text)
    parsed_dates = [parse_flight_date(d) for d in all_dates if parse_flight_date(d)]
    parsed_dates = sorted(set(d for d in parsed_dates if d))

    # ── Determine direction (Cairo→Riyadh or Riyadh→Cairo) ─────────
    direction = 'outbound'  # EGY→KSA = going onsite
    from_city = to_city = airline = flight_no = ''

    m_route = re.search(r'(Cairo|Riyadh|Jeddah|Dammam|Mecca)\s+\S+.*?(Cairo|Riyadh|Jeddah|Dammam)\s+\S+.*?([A-Z]{2}\d{2,4})', text, re.IGNORECASE|re.DOTALL)
    if m_route:
        from_city = m_route.group(1)
        to_city   = m_route.group(2)
        flight_no = m_route.group(3)
        if 'cairo' in from_city.lower():
            direction = 'outbound'  # EGY→KSA
        else:
            direction = 'return'    # KSA→EGY

    # Airline
    for airline_name in ['Saudia','EgyptAir','flydubai','Flynas','Air Arabia']:
        if airline_name.lower() in text.lower():
            airline = airline_name; break

    # ── Build result ────────────────────────────────────────────────
    start_date = parsed_dates[0] if parsed_dates else ''
    end_date   = ''  # Single ticket = one-way; end_date = return ticket

    # If this is a return ticket, the end_date might be further in text
    if len(parsed_dates) >= 2 and direction == 'outbound':
        # Second date could be return date on same ticket
        end_date = parsed_dates[-1] if parsed_dates[-1] != start_date else ''
